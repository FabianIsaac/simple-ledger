...
export class RecurringSidebarView extends ItemView {
	...
	getDisplayText(): string { return 'Recurrentes'; }
	...
	render(): void {
		...
		header.createEl('h3', { text: 'Recurrentes' });
		const headerBtns = header.createDiv('sl-header-btns');
		const addBtn = headerBtns.createEl('button', { text: '+ Nueva', cls: 'sl-header-btn mod-cta' });
		addBtn.addEventListener('click', () => {
			new AddRecurringModal(this.app, this.plugin, null, () => this.render()).open();
		});
		const creditBtn = headerBtns.createEl('button', { text: '🏦', cls: 'sl-header-btn', attr: { title: 'Nuevo credito' } });
		...
		this._miniCard(summaryCards, 'Pendientes', String(pendingCount), 'sl-rec-sb-warn');
		this._miniCard(summaryCards, 'Pagadas', String(paidCount), 'sl-rec-sb-ok');
		this._miniCard(summaryCards, 'Gastos/mes', fmtAmount(totalMonthly, settings), 'sl-rec-sb-expense');
		this._miniCard(summaryCards, 'Ingresos/mes', fmtAmount(totalIncomeMonthly, settings), 'sl-rec-sb-income');

		// Filter tabs
		const tabs = container.createDiv('sl-rec-sb-tabs');
		const tabDefs = [
			{ key: 'all', label: `Todas (${recs.length})` },
			{ key: 'pending', label: `Pendientes (${pendingCount})` },
			{ key: 'paid', label: `Pagadas (${paidCount})` },
			{ key: 'credits', label: `Creditos (${credits.length})` },
		];
		...
		sortRow.createSpan({ text: 'Ordenar: ', cls: 'sl-rec-sb-sort-label' });
		const sortOpts = [
	...
	private _renderRecurringList(container: HTMLElement, withStatus: { rec: RecurringTransaction; isPaid: boolean; nextDue: string }[]): void {
		...
		if (items.length === 0) {
			list.createEl('p', { text: 'Sin resultados', cls: 'sl-empty-msg sl-empty-small' });
			return;
		}
		...
		for (const item of items) {
			...
			if (isCredit) {
				nameDiv.createSpan({ text: 'Credito', cls: 'sl-rec-sb-badge sl-rec-sb-badge-credit' });
			}
			row1.createSpan({
			...
			if (!isPaid) {
				const payBtn = actions.createEl('button', { cls: 'sl-rec-pay-btn', attr: { title: 'Registrar pago' } });
				payBtn.innerHTML = '&#10003;';
